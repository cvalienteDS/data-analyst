...
Las películas más taquilleras de la historia son de acción, pero en promedio es un género que no recomendaríamos.
Drama y crimen son las más probables de obtener pérdidas.

Como dato curioso observamos que a lo largo de las décadas han aumentado los presupuestos para las películas de acción aventura y animación, manteniéndose invariables en otros géneros.
Parece como si la industria cinematográfica ya se hubiese dado cuenta antes de nuestro análisis, donde merece la pena invertir. El género de terror es un caso particular ya que no muestra una relación lineal, las películas con bajo presupuesto de terror han funcionado históricamente bien, mientras que las más caras tienden a dar pérdidas.
...
ggplot(dataHorror, aes(x=budget, y=netGross)) + 
  geom_point()+
  geom_text_repel(data = dataHorror, aes(label = name), max.overlaps = 7, verbose = F)+
  geom_smooth()
